...
app.get("/movies/:id", function (request, response) {
  ...
  const name = movies.find((movie) => movie.id == request.params.id);
  //   if(name){
  //   response.send (name)
  //   }
  //   else{
  //     response.send(NOT_FOUND_MSG)
  //   }
  name ? response.send(name) : response.status(404).send(NOT_FOUND_MSG);
});

app.get("/movies", function (request, response) {
  console.log(request.query.name);
  const search_query = request.query.name;
  const NOT_FOUND_MSG = { msg: "Movie not found" };

  const name = movies.find(
    (movie) => movie.name?.toLowerCase() === search_query?.toLowerCase()
  );

  name ? response.send(name) : response.status(200).send(movies);
});
